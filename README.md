# Re: BiliLiveNotification
## Re：B站直播开播提醒
[Windows](https://github.com/SadYuyuko/ReBiliLiveNotification/releases/download/v1.3/Re.BiliLiveNotification.exe) | [Android](https://github.com/SadYuyuko/ReBiliLiveNotification/releases/download/a1.3/Re.BLN_Android_1.3.apk) 下载  

修改内容：  
 - 开机自启
 - 自动开始检测  
 - 自动跳转浏览器  
 - 适配高分屏  
 - 删除启动通知，因为系统推送会留下一长串通知记录...  
 - 将`.ini`配置文件存放至`C:\Users\用户名\.ReBiliLiveNotification\ReBLN.ini`  
 - 自定义API（好像用不到...）  
 - kotlin移植Android版：直接跳转B站直播间界面  

注意：  
1.设置开机自启需要以管理员运行，移动.exe位置后要重新设置开机自启  
2.输入房间号后要重新开始检测或保存设置才会加入检测  
3.Android版自动逻辑：未点击停止检测时关闭APP，则下次打开时自动开始检测    
  
## 软件界面  
- Windows  
<img width="300" height="474" alt="1" src="https://github.com/user-attachments/assets/36bd72cc-3282-40b7-9e02-f8210af82340" />
<img width="250" height="172" alt="2" src="https://github.com/user-attachments/assets/ff246456-ad76-403c-a51c-3a4dfe58f524" />

- Android  
<img width="270" height="615" alt="1" src="https://github.com/user-attachments/assets/f0df93b5-e63f-41d7-b7a1-0688be507e45" />
<img width="270" height="615" alt="2" src="https://github.com/user-attachments/assets/1154962c-4644-4e26-be04-f34d2ea90740" />
